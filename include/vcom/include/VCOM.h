...
class VCOM {
public:
    VCOM() : uart_data_{-1}, is_running_(false), terminal_monitor_(true) {
    ...
    int findConnectableDevice(const std::string base_path = DEVICES_PATH); // 寻找可用设备

    void callBackFuncRegister(CallbackType callBackFunc, uint16_t func_code, uint8_t id);

    void transmit(std::vector<uint8_t> &data, uint8_t func_code, uint16_t id);
     ...
     */
    template <class T>
    void transmit(T package_data, uint8_t fun_code, uint16_t id) {
        ...
        }
        {
            ...
            transmit_queue_.push_back(tdata);
        }
    }
...
private:
    ...
    std::map<std::pair<uint16_t, uint8_t>, std::vector<CallBackFuncInfo>> callBackFuncMap_; // 储存回调函数
    ...
    std::deque<VCOMData> data_queue_; // 读取解包数据队列
    ...
    std::deque<VCOMData> transmit_queue_; // 待发送数据队列
};
}
