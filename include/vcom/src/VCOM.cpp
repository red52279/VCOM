...
int VCOM::processData() {
    ...
    while (++ptr < BUFFER_SIZE - 7) {
        ...
        vcom_data_buffer_.time = read_time;
        {
            ...
            data_queue_.emplace_back(vcom_data_buffer_);
        }
...
void VCOM::tryStartCallBackFunc() {
    ...
    for (VCOMData &qdata: data_queue_) {
        auto it_ = callBackFuncMap_.find(std::make_pair(qdata.func_code, qdata.id));
        if (it_ != callBackFuncMap_.end()) {
            for (CallBackFuncInfo &func: it_->second) {
                func.func(qdata.data.data());
...
void VCOM::transmitQueueFrontData() {
    ...
    transmit(transmit_queue_.front().data, transmit_queue_.front().func_code, transmit_queue_.front().id);
 ...
 */
void VCOM::callBackFuncRegister(CallbackType callBackFunc, uint16_t func_code, uint8_t id) { // 寻找id与func_code一致的回调函数并调用
