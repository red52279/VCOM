...
int VCOMCOMM::FindConnectableDeviceDir(const std::string base_path, device_data data) { // 递归读取设备信息
    ...
    if (base_path == DEVICES_PATH) {
        ...
        if (m_connectable_port_devices.empty()) {
            ...
            for (int i = 0; i < m_connectable_port_devices.size(); i++) {
            ...
            if (m_connectable_port_devices.size() == 1) {
                ...
                while (select < 1 || select > m_connectable_port_devices.size()) {
                    ...
                    for (int i = 1; i <= m_connectable_port_devices.size(); i++) {
                        std::cout << i;
...
void VCOMCOMM::Transmit(VCOMData data, uint8_t fun_code, uint16_t id) {
    uint8_t buff[8];
    std::vector<uint8_t> Data;
    for (int i = 0; i < 8; i++) {
        Data.push_back(*((uint8_t*)(&data) + i));
    }
    Transmit(Data, fun_code, id);
}

void VCOMCOMM::Transmit(std::vector<uint8_t> &data, uint8_t fun_code, uint16_t id) {
